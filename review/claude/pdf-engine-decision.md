...
- **The renderer and the logo provider are now wired — to an admin preview, and to nothing else.** `AdminController::previewPdf()` is the single construction site, behind a capability check and a per-document nonce; the provider is built with no arguments, so the logo is the site's current Custom Logo with nothing to configure. No mailer and no `DeliverDocument` is constructed anywhere, no order hook reaches the preview, and nothing is stored. **Email delivery is still not wired** and remains gated on the separate end-to-end sandbox stage. See `admin-preview-wiring.md`.
